...
//
// });
var campiPreventivoInvalidi;
var formTag = {"name":     {"need":true,  "touch":false, "i":0, "input":"input", "defVal":"","evt":["keyup","cut","paste","change"]},
               "company":  {"need":true,  "touch":false, "i":1, "input":"input", "defVal":"","evt":["keyup","cut","paste","change"]},
               "telephone":{"need":true,  "touch":false, "i":2, "input":"input", "defVal":"","evt":["keyup","cut","paste","change"]},
               "email":    {"need":true,  "touch":false, "i":3, "input":"input", "defVal":"","evt":["keyup","cut","paste","change"]},
               "message":  {"need":true,  "touch":false, "i":4, "input":"textarea", "defVal":"","evt":["keyup","cut","paste","change"]},
               "agree":    {"need":true,  "touch":false, "i":5, "input":"input", "defVal":false,"evt":["change"]},
               "modello":  {"need":false, "touch":false, "i":6, "input":"select", "defVal":"","evt":["change"], "bottoni":true},
               "tessuto":  {"need":false, "touch":false, "i":7, "input":"select", "defVal":"","evt":["change","click"]},
               "altezza":  {"need":false, "touch":false, "i":8, "input":"input", "defVal":"","evt":["keyup","cut","paste","change"]},
               "larghezza":{"need":false, "touch":false, "i":9, "input":"input", "defVal":"","evt":["keyup","cut","paste","change"]},
               "misura":   {"need":false, "touch":false, "i":10, "input":"select", "defVal":"","evt":["change","click"]},
               "struttura":{"need":false, "touch":false, "i":11, "input":"select", "defVal":"","evt":["change","click"]},
               "finitura": {"need":false, "touch":false, "i":12, "input":"select", "defVal":"","evt":["change","click"]},
               "base":     {"need":false, "touch":false, "i":13, "input":"select", "defVal":"","evt":["change","click"]},
               "borsa":    {"need":false, "touch":false, "i":14, "input":"input", "defVal":false,"evt":["change"]},
               "spedizione":{"need":false, "touch":false, "i":15, "input":"select", "defVal":"","evt":["change","click"]},
               "quantita": {"need":false, "touch":false, "i":16, "input":"input", "defVal":"","evt":["keyup","cut","paste","change"]} };
        ...
        if(key == "larghezza" || key == "altezza") {
          ...
          if(document.getElementById("modello")) {
            temp.addEventListener(formTag[key]["evt"][i], function(){
              document.getElementById("modello").getElementsByTagName(formTag[key]["input"]).value="tapp";
              deselezionaGruppo("modello");
              document.getElementById("modello").querySelectorAll('[data-options="tapp"]')[0].classList.add("selected");
            });
          }
        }
      }
      // focus e lostfocus per nascondere menu nei cellulari
      if((formTag[key]["input"] == "input" && typeof formTag[key]["defVal"] != "boolean") || formTag[key]["input"] == "textarea" ) {
...
async function sendMail() {
    ...
    gtag_report_conversion('https://www.grigoprint.it/contatti.html');
...
function calcolaPrezzo(inputId) {
  ...
  if (campiPreventivoInvalidi.length > 0) {
      ...
      scriviPrezzo("Inserisci "+campiPreventivoInvalidi[0], "Inserisci tutte le informazioni prodotto se vuoi un preventivo immediato, oppure scrivici nel modo clasisco");
      // aggiungo i campi WARN anche nelle caselle prima
      for(var i in campiPreventivoInvalidi) {
        if((formTag[campiPreventivoInvalidi[i]]["i"] < formTag[inputId]["i"]) || formTag[campiPreventivoInvalidi[i]]["touch"]) {
          if(i == 0)
            document.getElementById(campiPreventivoInvalidi[i]).classList.add("warning");
          document.getElementById(campiPreventivoInvalidi[i]).classList.add("warn");
        }
      }
      return;
  }
...
function calcolaPrezzoTappeti() {
    ...
    if (altezza > larghezza) {
        if (altezza <= 100) {
            ...
            scriviPrezzo("Richiedi in Azienda", "Misure troppo grandi, vi offriremo piu tappeti giuntati");
            return;
        }
    } else {
        if (larghezza <= 100) {
            ...
            scriviPrezzo("Richiedi in Azienda", "Misure troppo grandi, vi offriremo più tappeti giuntati");
            return;
        }
    }
...
function calcolaPrezzoTovaglie() {
    ...
    if (altezza > larghezza) {
        if (altezza <= 150) {
            ...
            scriviPrezzo("Richiedi in Azienda", "Misure troppo grandi, vi offriremo piu tessuti uniti.");
            return;
        }
    } else {
        if (larghezza <= 150) {
    ...
    // calcolo finale prezzo (cambiare il prezzo del listino)
    //         altezzaTessuto/metri lineari-->prezzo
    var listini = {150: {15:25,  10000:20.7},
                   300: {15:58.9,10000:50} };
    var prezzo = ml * listini[altezzaTessuto][getQuantitàListino(ml,listini[altezzaTessuto])];
...
function calcolaPrezzoBeachFlags() {
    ...
    if(tessuto == "bifacciale")
      prezzo *= 3;
    prezzo += listini[modello]["base"][base][getQuantitàListino(quantita,listini[modello]["base"][base])] +
...
function calcolaPrezzoCarSolutions() {
    ...
    var listini = {
          "poggiatesta": {
            "1": {49:"almeno 50 pz", 100:8.26, 200:5.43, 500:4.57 ,1000:4.2, 2000:3.84, 10000:3.56 },
            "2": {49:"almeno 50 pz", 100:9.31, 200:6.45, 500:5.76 ,1000:5.31, 2000:4.96, 10000:4.64 },
            "a": {49:"almeno 50 pz", 100:9.47, 200:7.58, 500:7.05 ,1000:6.56, 2000:6.2, 10000:5.81 }
          }
        };

    var prezzo = listini[modello][finitura][getQuantitàListino(quantita,listini[modello][finitura])];
...
function calcolaPrezzoCornice() {
    ...
    // calcolo finale prezzo (cambiare il prezzo del listino)
    //         altezzaTessuto/metri lineari-->prezzo
    var listini = {"avviamento":{"standard":        50,
                                 "retroilluminato": 50,
                                 "solo cucitura":   10 },
                   "stampa": {"standard":        {30:17,  10000:16},
                              "retroilluminato": {30:20.5,10000:19},
                              "solo cucitura":   {0:0}  },
                   "cucitura": {50:4, 10000:3}
        };
    var prezzo = listini["stampa"][finitura][getQuantitàListino(mq,listini["stampa"][finitura])] * mq +
                 listini["cucitura"][getQuantitàListino(perimetro,listini["cucitura"])] * perimetro;
...
function calcolaPrezzoOmbrelloniGiardino() {
    ...
    var listini = {
        "centrale": {
          "legno":{ "3x3":     [410,35],
                    ...
                    "3x4":     [530,40],
                    "4x4":     [680,45]  },
          "legno sbiancato":{ "3x3":     [490,35],
                              "3.5x3.5": [640,40],
                              "3x4":     [620,40],
                              "4x4":     ["Non disponibile",0]  },
          "alluminio":{ "3x3":     [480,35],
                        "3.5x3.5": [610,40],
                        "3x4":     [600,40],
                        "4x4":     ["Non disponibile",0]  }
        },
        "braccio": {
          "legno":{ "3x3": [960,100],
                    ...
                    "3x4": [1160,100],
                    "4x4": [1500,100]  },
          "legno sbiancato":{ "3x3": [1220,100],
                              "3.5x3.5": [1440,100],
                              "3x4": [1420,100],
                              "4x4": ["Non disponibile",100]  },
          "alluminio":{ "3x3": [1000,100],
                        "3.5x3.5": [1210,100],
                        "3x4": [1190,100],
                        "4x4": [1570,100]  },
          "retrattile": { "3x3": ["Non disponibile",0],
    ...
    if(typeof prezzo == "number") {
      ...
      if(spedizione == "spedizione") // se ritiro in sede non conteggio
...
function selezionaElemento(id){
  var valoreGiaSelezionato = document.getElementById(id).getElementsByTagName(formTag[id]["input"])[0].value;
  var elementoGiaSelezionato = document.getElementById(id).querySelectorAll("[data-options=\""+valoreGiaSelezionato+"\"]")[0];
  if (elementoGiaSelezionato) {// solo se ci sono "bottoni a" dentro
    // gestisco selezione
    deselezionaGruppo(id);
...
var mappaAggiornaSelect = {"modello":{"vela":{"misura":["|Seleziona una Misura","s|S, h finale cm250","m|M, h finale cm310","l|L, h finale cm400","xl|XL, h finale cm470"],"finitura":["|Seleziona una Finitura","normale|Con fettuccia Bianca o Nera nel retro","stampata|Con fettuccia stampata"],"base":["|Seleziona una Base","senza|Senza base","croce|Base a croce con zavorra riempibile","19|Base in resina bianca, riempibile, da 19L","40|Base in resina nera da 40L","d33|Base disco resina da 33cm, riempite di cemento","piastra|Base a piastra in metallo","vite|Vite in plastica","vitone|Vitone in ferro zincato","picchetto|Picchetto in ferro zincato"]},
                                       ...
                                       "personalizzata":{"tessuto":["poliestere|Poliestere"],"misura":["|Seleziona miusra","108|&Oslash;25cm x &#8596;108 cm","130|&Oslash;30cm x &#8596;130 cm","150|&Oslash;34cm x &#8596;150 cm","200|&Oslash;46cm x &#8596;200 cm","260|&Oslash;60cm x &#8596;260 cm","300|&Oslash;69cm x &#8596;300 cm","350|&Oslash;80cm x &#8596;350 cm","400|&Oslash;92cm x &#8596;400 cm"]},
                                       "non personalizzata":{"tessuto":["|Seleziona tessuto","acrilico|Acrilico","poliestere|Poliestere"],"misura":["|Seleziona miusra","108|&Oslash;25cm x &#8596;108 cm","130|&Oslash;30cm x &#8596;130 cm","150|&Oslash;34cm x &#8596;150 cm","200|&Oslash;46cm x &#8596;200 cm","260|&Oslash;60cm x &#8596;260 cm","300|&Oslash;69cm x &#8596;300 cm","350|&Oslash;80cm x &#8596;350 cm","400|&Oslash;92cm x &#8596;400 cm"]},
                                       ...
                                       "centrale":{"misura":["|Seleziona una Misura","3x3|3x3 metri","3.5x3.5|3.5x3.5 metri","3x4|3x4 metri","4x4|4x4 metri"], "struttura":["|Seleziona una Struttura","legno|Legno classico","legno sbiancato|Legno Sbiancato","alluminio|Alluminio"], "base":["|Seleziona una Base","senza|Senza base","55|55 Kg","75|75 Kg","70|70 Kg","100|100 Kg"]},
                                       "braccio":{"misura":["|Seleziona una Misura","3x3|3x3 metri","3.5x3.5|3.5x3.5 metri","3x4|3x4 metri","4x4|4x4 metri"], "struttura":["|Seleziona una Struttura","legno|Legno classico","legno sbiancato|Legno Sbiancato","alluminio|Alluminio","retrattile|Retrattile"], "base":["|Seleziona una Base","senza|Senza base","2piastrelle| 2 zavorre in graniglia da 50Kg cad."]},
                                       "pergola":{"misura":["|Seleziona una Misura","3x3|3x3 metri"], "struttura":["alluminio|Alluminio"], "base":["|Seleziona una Base","senza|Senza base","4piastrelle| 4 piastrelle in graniglia da 50Kg cad.","8piastrelle| 8 zavorre in graniglia da 50Kg cad."]}  }};
function mostraNascondiSelect(id,select) {
  var finalValue;
  for(var key in mappaAggiornaSelect[id][select]) {
    formTag[key]["touch"] = false;
    if(!isNaN(parseInt(mappaAggiornaSelect[id][select][key]))) { // compilo e bloco campi numeri
      ...
      if(mappaAggiornaSelect[id][select][key].length <= 1) {
        document.getElementById(key).classList.add("blocca");
        document.getElementById(key).getElementsByTagName("select")[0].disabled = true;
      } else {
        document.getElementById(key).classList.remove("blocca");
        document.getElementById(key).getElementsByTagName("select")[0].disabled = false;
      }
    ...
    document.getElementById(key).getElementsByTagName(formTag[key]["input"])[0].value = finalValue;
    if(formTag[key]["need"])
